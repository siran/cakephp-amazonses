 ...
 *
 * Based on Lucas Ferreira's (Burn web.studio - http://www.burnweb.com.br/) implementation of the 
 * original class from: http://bakery.cakephp.org/articles/dankroad/2011/01/30/integrate_amazon_simple_email_service_ses_into_existing_application
 * 
 * Usage:
 * 
 ...
 * @copyright Copyright 2011, Viorel Sfetea - http://viorel.sfetea.ro/
 * @version 1.1b
 */
...
class EmailServiceComponent extends EmailComponent { 
	...
	function _aws_ses_raw() {
		...
		if(!$ok) { 
			$this->log('Error sending raw email from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function _aws_ses() {
		...
		if ( is_array( $this->to ) ) {
			$destinationEmails = $this->to;
		} else {
			$destinationEmails = explode(',', $this->to);
		}
		...
		if(!$ok) {
			$this->log('Error sending email from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function verifyEmailAddress($email) {
		...
		if(!$ok) {
			$this->log('Error verify email address from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function deleteVerifiedEmailAddress($email) {
		...
		if(!$ok) {
			$this->log('Error delete_verified_email_address from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function listVerifiedEmailAddresses() {
		...
		$ok = $this->response->isOK(); 
		if(!$ok) {
			$this->log('Error list_verified_email_addresses from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function getSendQuota() {
		...
		if(!$ok) {
			$this->log('Error get send quota from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
	...
	function getSendStatistics() {
		...
		if(!$ok) {
			$this->log('Error get send statistics from AWS SES: ' . $this->response->body->asXML(), 'awsses'); 
		}
